...
<body>
	...
	<!-- 员工添加模态框 -->
	<div class="modal fade" id="whitelist_add_modal" tabindex="-1"
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal">
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="path_input"
									placeholder="path" name="path"> <span
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="description_input"
									placeholder="description" name="description"> <span
									class="help-block"></span>
							</div>
						</div>
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary"
						id="whitelist_save_btn">保存</button>
				</div>
			</div>
		</div>
	</div>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
					<button type="button" class="btn btn-primary"
						id="whitelist_update_btn">更新</button>
				</div>
			</div>
		</div>
	</div>
				...
				<div class="box-body">
					<div id="example1_wrapper"
						...
						<div class="row">
							...
							<div class="col-sm-8">
								<form action="/strong/whitelist/list" method="get"
									id="search_form">
									...
									&nbsp;&nbsp;&nbsp;&nbsp;
									<button type="button" id="whitelist_add_modal_btn"
						...
						<!--<div class="col-sm-1">
								<div id="example1_filter" class="dataTables_filter">
									<button type="button" id="whitelist_add_modal_btn" style="width: 100%;" class="btn btn-inline btn-success btn-sm">添加</button>
								</div>
					...
					<div class="row">
						<div class="col-sm-12">
							<table id="interfaces_table"
								...
								<tbody>
									<c:forEach var="whitelist" items="${requestScope.whitelists }">
										<tr role="row">
											<td>${whitelist.id }</td>
											<td>${whitelist.path }</td>
											<td>${whitelist.description }</td>
											<td>${whitelist.updateTime }</td>

											<td><button type="button"
													style="width: 45%; float: left;"
													class="btn btn-block btn-danger btn-xs">删除</button>
												<button type="button" edit-id="${whitelist.id }"
								...
								<ul class="pagination">
									<c:if test="${page.hasPreviousPage }">
										<li class="paginate_button previous" id="example1_previous"><a
											href="/strong/whitelist/list?page=1" aria-controls="example1"
											data-dt-idx="0" tabindex="0">首页</a></li>

										<li class="paginate_button previous" id="example1_previous"><a
											href="/strong/whitelist/list?page=${page.pageNum-1 }"
									...
									<c:forEach items="${page.navigatepageNums }" var="page_num">
										...
										<c:if test="${page_num==page.pageNum }">
											<li class="paginate_button active"><a
												href="/strong/whitelist/list?page=${page_num }"
									...
									<c:if test="${page.hasNextPage }">
										...
										<li class="paginate_button next" id="example1_next"><a
											href="/strong/whitelist/list?page=${page.pages }"
...
<script type="text/javascript">
	$(function() {
		...
		$("[href='/strong/whitelist/list']").parent().addClass("active");
		...
		$("#whitelist_add_modal_btn").click(function() {
			$("#whitelist_add_modal").modal({
				backdrop : "static"
			});
		});
		...
		$("#whitelist_save_btn")
				.click(
						function() {
							...
							$
									.ajax({
										url : "/strong/whitelist/add",
										data : $("#whitelist_add_modal form")
												.serialize(),
										type : 'post',
										success : function(result) {
											if (result.code == 1) {
	...
	function getScheme(id) {
		$.ajax({
			url : "/strong/whitelist/getWhiteListById?id=" + id,
			type : 'get',
			success : function(result) {
	...
	function delete_scheme(object) {
		...
		if (!confirm("确认要删除  " + $(object).parent().siblings().eq(1).text()
			...
			return false;
		$.ajax({
			url : "/strong/whitelist/deleteById?id=" + id,
			type : "get",
			success : function(result) {
	...
	function build_page_nav(result) {
		...
		$.each(result.map.pageInfo.navigatepageNums, function(index, page) {
			var numLi = $("<li></li>").addClass("paginate_button")
					.append(
							$("<a></a>").attr("href",
									"/strong/whitelist/list?page=" + page)
									.append(page));
			if (pageNum == page) {
